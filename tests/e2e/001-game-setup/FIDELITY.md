...
The E2E stories cover 2/3/4 seats; whitespace validation and a long name; keyboard submission, dialogs, Escape and focus restoration; actual invitation copying and selectable manual copying; remote arrivals and exact motion counts; restoration without a remembered name; competing guests for the last seat; full/missing/invalid invitations; failed initial sign-in retry; and interrupted table recovery. The added story joins through Play using the displayed code, accepts lowercase, rejects an empty code, checks guest permissions, observes live capacity and supply changes, and restores resized tables. A lost creation-response story commits a real transaction, drops its acknowledgement, and verifies recovery with the same code and exactly one stored event. Public game events and membership are real emulator data. The room-code entropy is fixed per story and viewport, with that fixture’s previous emulator table removed before creation; no visible text is masked or replaced. Clipboard failure and transport interruptions are the only injected platform failures.

...
export type SetupEvent = {
  ...
  sequence: number;
  actorUid: string;
  type: 'game/created' | 'player/joined';
  name: string;
  playerCount: 2 | 3 | 4;
};
...
export function replaySetup(events: SetupEvent[]): SetupState {
  ...
  for (const event of [...events].sort((a, b) => a.sequence - b.sequence)) {
    if (event.schemaVersion !== 1 || event.sequence !== state.players.length + 1 ||
      ![2, 3, 4].includes(event.playerCount) || !event.actorUid ||
      typeof event.name !== 'string' || !event.name.trim() || event.name.length > 24 ||
      state.players.some(player => player.uid === event.actorUid)) throw new Error('Invalid setup event stream.');
    if (event.sequence === 1) {

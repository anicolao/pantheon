...
| **004 · Receive my hand** | Final leader → deal → private table (03–04) | Exactly 6 Obols, 3 Hamlets, matching Temple; draw five, deck five, no discard; first seat starts 1/1/1/0. Return never redeals. Opponents’ faces/order are not rendered in the DOM or accessibility tree. Trusted clients replay the complete state from the shared seed; network-level secrecy is not part of this model. |
...
| **017 · Share a table privately** | Open shared view → public table → private controller → public move → end viewing access (16,19) | Separate player/viewer contexts, 4K/portrait fit, no hidden faces rendered to viewer, viewer denied moves, common backs, no seat transferred by invitation, revocation enforced. Private controller is normal 04, not another seat. |
...
These are implementation requirements, never UI copy. Trusted clients synchronize through the shared event stream, with deterministic randomness from a recorded initial seed and complete client replay. Versioned reducers validate game commands and derive all decks and hands. Each player-facing view renders only its owner’s hand; opponents use common backs, including accessibility descriptions. This is presentation privacy, not secrecy from a participant inspecting the shared seed or replay state. A shared display needs a revocable read-only capability with no move authority. An invitation does not transfer player identity.
